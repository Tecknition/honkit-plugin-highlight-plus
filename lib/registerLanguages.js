  ...
  languages.forEach((lang) => {
    ...
    try {
      ...
      const modulePath = require.resolve(
        `highlight.js/lib/languages/${languageName}.js`,
      );
      const langDef = require(modulePath);
      hljs.registerLanguage(languageName, langDef);
      console.log(
        `[highlight-plus] Registered custom language: ${languageName}`,
      );
    } catch (err) {
      console.warn(
        `[highlight-plus] Could not register language: ${languageName}`,
        err.message,
      );
    }
  });
