...
function loadThemeCss(theme) {
  ...
  try {
    ...
    console.warn(
      `[highlight-plus] Unable to load theme '${theme}':`,
      err.message,
    );
...
function highlightSnippet(code, language, hljs) {
  ...
  try {
    ...
    console.warn(
      `[highlight-plus] Highlight failed for '${language}':`,
      err.message,
    );
    ...
    (match, infoString, rawCode) => {
      ...
      const tableRows = lineNumbers
        ? lines
            .map(
              (line, index) =>
                `<tr><td class="line-number">${index + 1}</td><td class="code-line">${line}</td></tr>`,
            )
            .join("\n")
